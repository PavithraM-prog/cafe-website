...
export default function CartPage() {
  ...
  const handleApplyCoupon = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!couponCodeInput.trim() || applyingCoupon) return;
                  ...
                  {coupon ? (
                    ...
                    <form onSubmit={handleApplyCoupon} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 rounded-full border border-borderColor bg-[#FFF8E7]/10 focus:bg-white px-4 py-2.5 text-xs text-foreground focus:border-accent transition-all uppercase shadow-sm focus:ring-1 focus:ring-accent font-semibold disabled:opacity-50"
                      />
                      <button
                        type="submit"
                        disabled={applyingCoupon}
                        className="rounded-full bg-secondary hover:bg-borderColor/50 text-foreground text-xs font-bold uppercase tracking-widest px-5 py-2.5 transition-colors border border-borderColor shadow-sm disabled:opacity-50 flex items-center justify-center space-x-1.5"
                      >
